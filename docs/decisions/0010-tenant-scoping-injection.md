...
**2026-05-27 — [DEL-22](https://linear.app/oveglobal/issue/DEL-22):** BA tenant resolver becomes brand-optional per [ADR-0012](./0012-storefront-brand-tenant-food-hall-architecture.md). `StorefrontTenantContext` widens to `{ tenantId, storefrontId, storefrontType, storefrontSlug, brandId?, brandSlug? }`; the resolver now calls `resolveStorefrontBySlug` (DEL-19 / DEL-20) and 400s only when no tenant resolves. Absence of `brandId` is no longer an error. Adapter writes are conditional on `brandId` presence: `current_brand_id` (DEL-21) and `brand_id` on verification rows (this issue), both via the explicit `ctx.brandId ?? null` pattern. **`SCOPED_MODELS` + `tenantId`-predicate behavior unchanged** — the security boundary is preserved; this amendment is purely about the brand-context surface. Tenant-host OTP / password-reset emails ship in DEL-22 with branding sourced from `storefronts.branding_json` + `tenants.name`/`logo` (fallback chain documented in [`docs/specs/ba-brand-optional.md`](../specs/ba-brand-optional.md)). `@rp/emails` event Zod schemas use backwards-compatible `z.union` shapes so legacy mode-less brand payloads still parse. The URL rewriter input is renamed `brandSlug` → `storefrontSlug` (semantic shift; same logic). OAuth tenant-host remains out of scope (AC#7 covers brand-host regression only). See [`docs/specs/ba-brand-optional.md`](../specs/ba-brand-optional.md).

**2026-05-28 — session-model-scoped (no Linear issue — small defense-in-depth fix; partial closure):** narrows the cross-tenant cookie-replay gap surfaced during DEL-26. Decision #1 above ("session lookups by token pass through") was incomplete: BA's `getSession` does a relational lookup that joins user data inline, so the wrapped adapter's user-side tenant predicate never fires as a separate hop. Partial closure (two coordinated changes): (1) `tenant_end_user_sessions` gets a `tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE` column (migration `0008_slimy_wrecking_crew.sql`, backfilled from `tenant_end_users.tenant_id` then SET NOT NULL); (2) BA config registers `session.additionalFields.tenantId` (`input: false`), `SCOPED_MODELS` extends to `{user, verification, account, session}`, and the wrapped adapter's `session.create` stamps `tenantId: ctx.tenantId` alongside `currentBrandId`. **`cookieCache` stays enabled** with the same 5-min TTL — discovered mid-implementation that disabling it would force every request through the adapter, but BA's `get-session` short-circuits the adapter on cookieCache hits anyway, and the same disable broke `food-hall.spec.ts` because Next.js 16's post-server-action-redirect page render drops the storefront subdomain from `Host` (documented inline at `apps/storefront/src/app/(shop)/orders/[orderId]/page.tsx`). Cross-tenant cookie replay during the 5-min cache window therefore still returns the source-tenant payload; the SCOPED_MODELS extension applies on cache misses + other adapter-routed session calls (delete-session-by-token, list sessions, etc.). The schema + write-side stamping still ship — every new session row carries `tenant_id`, positioning a follow-up to close the cookieCache path properly (via `cookieCache.version` callback returning current tenant, or upstream Next.js fix). Cookie-isolation cross-tenant replay tests remain dropped; restored when the follow-up lands. Same-tenant cross-brand recognition (auth.spec.ts DEL-7 + DEL-14 + sibling-brand) and food-hall mode-3 flow unchanged. See [`docs/specs/session-model-scoped.md`](../specs/session-model-scoped.md) § Open Questions §1 for the follow-up plan.

**2026-05-27 — cookie-cache-tenant-version (Phase 3 M1):** closes the cookieCache cross-tenant cookie-replay gap left open by session-model-scoped. Three coordinated changes: (1) `packages/auth-core/src/storefront.ts` registers `session.cookieCache.version` as an async callback that calls the closure-captured `resolveTenantContext()` and returns `ctx.tenantId`. BA invokes this at both cache-write time (`dist/cookies/index.mjs:69-86`, post-signup/signin) and cache-read time (`dist/api/routes/session.mjs:93-104`, every `get-session` cookieCache hit). At write time the cached payload's `version` field becomes the writer-tenant's `tenantId`; at read time the callback returns the reader-tenant's `tenantId`. Mismatch on cross-tenant replay → BA `expireCookie` → falls through to `internalAdapter.findSession(token)` → wrapped adapter's tenant predicate (session-model-scoped) rejects → BA returns null user. (2) `apps/storefront/src/lib/storefront-tenant-context.ts` adds a 4-source precedence chain — `x-storefront-id` header (proxy-injected UUID, trusted only because the proxy strips client-supplied `PROXY_OWNED_HEADERS` before its own injection) → `Host` + `extractStorefrontSlug` → `Referer` URL host + extractor → `Origin` URL host + extractor. The resolver throws `APIError('BAD_REQUEST', 'TENANT_CONTEXT_REQUIRED')` only when all four sources fail. The export is wrapped in React `cache()` so per-render duplicate invocations dedupe (implementation goal, not a hard test gate per spec § AC#5). (3) `apps/storefront/src/proxy.ts` adds a matching Referer/Origin fallback on bare-host page renders — `/api/*` short-circuit ordering is **preserved** (strip PROXY_OWNED_HEADERS → short-circuit `/api/*` before any injection — moving the injection earlier would carry an unneeded slug-resolution penalty on every BA call). The Referer fallback covers the Next.js 16 post-server-action-redirect render path that previously required cookieCache-on to mask. Cross-tenant cookie-replay tests restored in `apps/storefront/tests/e2e/cookie-isolation.spec.ts` (tests 2 + 3, both directions). No schema changes — `tenant_end_user_sessions.tenant_id` from session-model-scoped is what the predicate uses. Same-tenant cross-brand + food-hall mode-3 flow unchanged. See [`docs/specs/cookie-cache-tenant-version.md`](../specs/cookie-cache-tenant-version.md). § 5.2 row #3 in [`storefront-tenant-scoping.md`](../specs/storefront-tenant-scoping.md) updated; the cookieCache-hit carve-out is removed.
