...
- [ ] `curl -H 'Host: pizza-express.localhost:3001' http://localhost:3001/` returns a 2xx page (not the "No brand specified" dev error). Confirms `getBrandContext('pizza-express')` resolves against the JOIN at [apps/storefront/src/lib/tenant-resolution.ts:73-97](../../apps/storefront/src/lib/tenant-resolution.ts). *(Best-effort.)*
