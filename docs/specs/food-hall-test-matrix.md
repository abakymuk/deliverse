...
| **cookie-isolation.spec.ts** (new — DEL-26) | ⚠️ Domain config (storefront-agnostic) | ⚠️ Domain config (storefront-agnostic) | ⚠️ Domain config (storefront-agnostic) | password (HTTP) | hospitality-group (canonical) | AC#6 | DEL-26 |
...
### Intentional Deviation from initial DEL-26 scope — cross-tenant session-replay tests

An earlier iteration of this spec included two additional cases: brand-host → tenant-host and tenant-host → brand-host cookie replay, asserting the BA `get-session` endpoint rejects a cross-tenant cookie at the **server side**. Running those tests against `dev` exposed a real defense-in-depth gap (documented in § "Open Questions §2") rather than a regression — the wrapped adapter at [`packages/auth-core/src/storefront-adapter.ts:72`](../../packages/auth-core/src/storefront-adapter.ts:72) excludes `session` from `SCOPED_MODELS` by design ([`docs/specs/storefront-tenant-scoping.md`](./storefront-tenant-scoping.md) §5, original DEL-3 contract). A server-side cookie replay from tenant A is accepted at tenant B and returns tenant-A's user+session payload.

AC#6 wording ("cookies scoped to exact storefront slug") refers to the cookie's `Domain` attribute — the browser-side guard — and is fully covered by the single test above. The server-side cross-tenant rejection is a separate invariant that doesn't hold today by design; closing that gap is captured as a follow-up task (see § "Open Questions §2"). Shipping `.fixme()`-marked tests in DEL-26 would muddy the PR; the follow-up task can land the tests alongside the SCOPED_MODELS fix.
...
2. **Server-side cross-tenant session-cookie replay gap (defense-in-depth).** DEL-26's first cookie-isolation iteration exposed a real gap: a session cookie minted at storefront A is accepted by BA's `/api/auth/get-session` at storefront B (different tenant) and returns tenant-A's user + session payload. Root cause: [`packages/auth-core/src/storefront-adapter.ts:72`](../../packages/auth-core/src/storefront-adapter.ts:72) declares `SCOPED_MODELS = {'user', 'verification', 'account'}` — `session` is intentionally excluded per the comment at line 9 (the original DEL-3 contract: session lookups by token pass through unscoped because session rows reference user via `tenant_end_user_id` FK, and the user lookup is scoped). In practice, the user lookup IS scoped, but BA's `getSession` returns the session row's joined user data without re-scoping by current tenant — so cross-tenant replay returns the wrong user. Threat model is info disclosure (victim's profile + brand/tenant context exposed when the cookie reaches the wrong server). Closing the gap requires adding `session` to `SCOPED_MODELS` and verifying BA's session-lookup paths still work. Captured as a separate follow-up task (spawned 2026-05-28). When that lands, restore the cross-tenant replay tests (initial draft preserved in the DEL-26 PR history) and flip this Open Question to a Decisions Log entry.
