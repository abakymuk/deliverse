...
**Partially closed 2026-05-28 by session-model-scoped.** Migration `0008_slimy_wrecking_crew.sql` added `tenant_id` to `tenant_end_user_sessions` and `SCOPED_MODELS` extended to include `session` — but BA's `cookieCache` (enabled, 5-min TTL) short-circuits the adapter on `get-session` cache hits, so the predicate doesn't apply for that path. The cross-tenant cookie-replay tests therefore remain dropped — they pass on cache misses but fail during the 5-min cache window. Restoring them lands with the follow-up that closes the cookieCache path (see [`session-model-scoped.md`](./session-model-scoped.md) § Open Questions §1). The Domain-attribute check (test 1) still passes and ships in this iteration.
...
2. **Server-side cross-tenant session-cookie replay gap (defense-in-depth) — partially closed 2026-05-28 by session-model-scoped.** Migration `0008_slimy_wrecking_crew.sql` added `tenant_id` to `tenant_end_user_sessions`; `SCOPED_MODELS` extended to include `session`; write-side stamping in place. But BA's `cookieCache` (enabled, 5-min TTL) short-circuits the adapter on `get-session` cache hits — cross-tenant replay during the cache window still returns the source-tenant payload. The cross-tenant replay tests stay dropped until a follow-up closes the cookieCache path. See [`session-model-scoped.md`](./session-model-scoped.md) § Open Questions §1 for the two viable closure paths (BA `cookieCache.version` callback returning current tenant, or upstream Next.js fix for the post-redirect Host drop that's currently blocking a cookieCache disable).
