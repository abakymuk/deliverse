...
| `session` | `findOne`, `findMany`, `count`, `update`, `updateMany`, `delete`, `deleteMany`, `consumeOne` | **Always** `{ field: 'tenantId', value: ctx.tenantId, operator: 'eq', connector: 'AND' }` (since session-model-scoped, post-DEL-26). **Caveat:** BA's `cookieCache` (enabled, 5-min TTL) short-circuits the adapter on `get-session` cache hits — the predicate then doesn't apply for that path. See [`docs/specs/session-model-scoped.md`](./session-model-scoped.md) § Open Questions §1 for the planned follow-up. | `get-session` on a cookieCache hit (5-min window per session). |
