...
| `session` | `findOne`, `findMany`, `count`, `update`, `updateMany`, `delete`, `deleteMany`, `consumeOne` | **Always** `{ field: 'tenantId', value: ctx.tenantId, operator: 'eq', connector: 'AND' }` (since session-model-scoped, post-DEL-26; cookieCache cross-tenant gap fully closed 2026-05-27 by [`cookie-cache-tenant-version.md`](./cookie-cache-tenant-version.md) via the BA `session.cookieCache.version` callback — on cache hits, a cross-tenant `version` mismatch expires the `session_data` cookie and falls through to this predicate). | n/a — predicate applies on every `get-session` (cookieCache hit → version eviction → DB lookup → predicate; cookieCache miss → DB lookup → predicate) and on every other adapter-routed session call. |
...
**2026-05-28 — session-model-scoped (no Linear issue — small defense-in-depth fix; partial closure):** narrows the session pass-through carve-out. `SCOPED_MODELS` now also includes `session`; wrapper stamps `tenantId` on `session.create` and appends the tenant predicate to all session reads/mutations that go through the adapter. Schema migration `0008_slimy_wrecking_crew.sql` adds `tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE` to `tenant_end_user_sessions` (backfilled from `tenant_end_users.tenant_id` via JOIN, then SET NOT NULL — matches DEL-12 migration pattern). BA config gains `session.additionalFields.tenantId` (`input: false`). **Known limitation (closed 2026-05-27, see next Amendment):** BA's `cookieCache` stayed enabled (5-min TTL), and `cookieCache` short-circuited the adapter on `get-session` cache hits — so the new tenant predicate only applied on cache misses + non-`get-session` paths. Discovered during DEL-26 cookie-isolation work; full root-cause analysis + migration sanity-check SQL in the spec.

**2026-05-27 — cookie-cache-tenant-version (Phase 3 M1):** closes the cookieCache cross-tenant cookie-replay gap left open by session-model-scoped. BA config gains `session.cookieCache.version` as an async callback that calls the closure-captured `resolveTenantContext()` and returns `ctx.tenantId` — invoked at both cache-write time (post-signup/signin) and cache-read time (every `get-session` cookieCache hit). Cross-tenant cookie replay during the cache window now mismatches the cached `version` field → BA `expireCookie` → falls through to the wrapped adapter → tenant predicate from session-model-scoped rejects → BA returns null user. The bare-host post-server-action-redirect render path (Next.js 16 quirk that previously blocked a cookieCache disable) is handled by a 4-source precedence chain in `apps/storefront/src/lib/storefront-tenant-context.ts` (`x-storefront-id` header → `Host` → `Referer` URL → `Origin` URL) plus a matching `x-storefront-id` injection in `apps/storefront/src/proxy.ts` on bare-host page renders. The proxy preserves its `/api` short-circuit ordering — `PROXY_OWNED_HEADERS` strip remains before injection (defense-in-depth against client-supplied spoofing). Cross-tenant cookie-replay e2e tests originally deferred during DEL-26 + session-model-scoped are restored as tests 2 + 3 in `apps/storefront/tests/e2e/cookie-isolation.spec.ts`. `SCOPED_MODELS` + tenant-predicate behavior **unchanged** — this amendment is about the cookieCache hit path closure + bare-host fallback. See [`docs/specs/cookie-cache-tenant-version.md`](./cookie-cache-tenant-version.md).
