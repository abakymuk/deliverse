...
| 6 | **`account` model NOT wrapped** in DEL-3. | `tenant_end_user_accounts(provider_id, account_id)` is globally unique per [`schema.ts:569`](../../packages/db/src/schema.ts) — a Google account can only link to one tenant_end_user globally. Adapter scoping alone can't fix this; needs a schema change. Punted to **DEL-3a** which blocks DEL-7 OAuth signup. | `packages/db/src/schema.ts:569` |
...
| `account` | — | **Not wrapped.** | See decision #6; DEL-3a addresses. |
